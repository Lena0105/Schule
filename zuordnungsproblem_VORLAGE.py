...
def loesungFinden3():
    global verbindungen, taxis, kunden,besteGesamtDistanz, abstaende,vergebeneKunden
    vergebeneKunden = []
    for i in range(len(kunden)):
        vergebeneKunden.append(False)
        
    besteVerbindungen = []
    for taxiIndex in range(len(taxis)):
        besteVerbindung = 10000
        indexBesterKunde = -99999
        for kundeIndex in range(len(kunden)):
            if vergebeneKunden[kundeIndex] == False:
                aktVerbindung = abstand(taxis[taxiIndex],kunden[kundeIndex])
                if aktVerbindung < besteVerbindung:
                    besteVerbindung = aktVerbindung
                    indexBesterKunde = kundeIndex
        besteVerbindungen.append([taxiIndex,indexBesterKunde])
        vergebeneKunden[indexBesterKunde] = True
    verbindungen = besteVerbindungen
    besteGesamtDistanz = berechneGesamtDistanzVerbindungen()
    print("BesteGesamtDistanz: ",besteGesamtDistanz)
    print("besteVerbindungen: ",besteVerbindungen)
    verbindungenZeichnen(besteVerbindungen)
