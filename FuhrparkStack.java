...
public class FuhrparkStack {
	...
	public int anzahlMitMaxBeladung(double pGewicht){
		int ergebnis = 0;
		Stack<Lastwagen> hilfsStack = new Stack<>();
		while(lastwagenStack.isEmpty() == false) {
			Lastwagen akt = lastwagenStack.top();
		...
		while(hilfsStack.isEmpty()==false) {
			Lastwagen aktL = hilfsStack.top();
			lastwagenStack.push(aktL);
			hilfsStack.pop();
		}
		return ergebnis;
	}
	...
	public Lastwagen lastwagenSuchen(double pGewicht){
		...
		while(lastwagenStack.isEmpty()== false) {
			Lastwagen akt = lastwagenStack.top();
			if(akt.getBeladung() != akt.getMaxBeladung()) {
				akt.zuladen(pGewicht);
				lastwagenStack.pop();
				return akt;
			}
			hilfsStack.push(akt);
